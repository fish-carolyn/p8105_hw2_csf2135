...
_Note that plastic bag collection data is only reported for Gwynnda Trash Wheel. Glass bottle, grocery bag, and chip bag collection data are reported for Mr. Trash Wheel and Professor Trash Well. Sports ball collection data are reported for Mr. Trash only._

For available data, what was the total weight of trash collected by Professor Trash Wheel? 

_The total weight of trash collected by Professor Trash Wheel was `r filter(all_trash, dumpster_name == "Prof. Trash Wheel") |> round(sum(weight_tons), digits =2)` tons._

What was the total number of cigarette butts collected by Gwynnda in July of 2021?

_In July of 2021. Gwynnda collected `r filter(all_trash, dumpster_name == "Gwynnda Trash Wheel", month == "July", year == 2021) |> round(sum(cigarette_butts), digits =2)` cigarette butts._
...
baseline_df <- read_csv("data_mci/MCI_baseline.csv", skip = 1) |> 
  janitor::clean_names() |> 
  case_match(sex, 1 ~ "Male", 0 ~ "Female") |>  
  recode (apoe4, 1 ~ "apoe4 carrier", 0 ~ "apoe4 non-carrier") |> 
